...
struct VerificationKeyTests {
    ...
    func testVerificationKeyHash() throws {
        let verificationKey = try VerificationKey(payload: Data(repeating: 0xAB, count: 32))
    ...
    func testVerificationKeyEquality() throws {
        ...
        let key1 = try VerificationKey(payload: payload)
        let key2 = try VerificationKey(payload: payload)
    ...
    func testVerificationKeyInequality() throws {
        let key1 = try VerificationKey(payload: Data(repeating: 0x01, count: 32))
        let key2 = try VerificationKey(payload: Data(repeating: 0x02, count: 32))
