...
struct RedeemerTests {
    ...
    func testRedeemerWithMyTestData() async throws {
        ...
        #expect(
            hex == "840000d8668218829f187b433233349f040506ffa2014131024132ff821a000f42401a000f4240")
    ...
    @Test("Test Redeemer uses compact constructor tag encoding")
    func testRedeemerCompactConstructorTagEncoding() throws {
        let redeemers: Redeemers = .list([
            Redeemer(
                tag: .spend,
                index: 0,
                data: .constructor(Constr(tag: 20, fields: [])),
                exUnits: ExecutionUnits(mem: 87_588, steps: 25_944_366)
            )
        ])

        let encoded = try CBOREncoder().encode(redeemers)
        let hex = encoded.hexEncodedString()

        #expect(hex == "81840000d9050d80821a000156241a018be12e")
    }

    @Test("Test Redeemer compact constructor tag round-trip")
    func testRedeemerCompactConstructorTagRoundTrip() throws {
        let expectedHex = "81840000d9050d80821a000156241a018be12e"
        let encoded = Data(hex: expectedHex)

        let decoded = try CBORDecoder().decode(Redeemers.self, from: encoded)
        let reencoded = try CBOREncoder().encode(decoded)

        #expect(reencoded.hexEncodedString() == expectedHex)

        guard case .list(let items) = decoded, let first = items.first else {
            Issue.record("Expected redeemers list with one item")
            return
        }

        guard case .constructor(let constr) = first.data else {
            Issue.record("Expected constructor plutus data")
            return
        }

        #expect(constr.tag == 20)
        #expect(constr.fields.isEmpty)
    }

    @Test("Test constructor tag 1293 decodes and re-encodes as constructor 20")
    func testCompactConstructorPayloadRoundTrip() throws {
        let expectedHex = "d9050d80"
        let encoded = Data(hex: expectedHex)

        let decoded = try PlutusData.fromCBOR(data: encoded)
        let reencoded = try decoded.toCBORData()

        #expect(reencoded.hexEncodedString() == expectedHex)

        guard case .constructor(let constr) = decoded else {
            Issue.record("Expected constructor plutus data")
            return
        }
    ...
    @Test("Test Redeemers JSON renders plutus data as constructor object")
    func testRedeemersJSONRendersPlutusDataObject() throws {
        let redeemers: Redeemers = .list([
            Redeemer(
                tag: .spend,
                index: 0,
        ...
        let json = try redeemers.toJSON() ?? ""

        #expect(json.contains("\"data\":{\"constructor\":20,\"fields\":[]}"))
        #expect(!json.contains("cborTag("))
    }
