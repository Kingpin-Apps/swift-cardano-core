...
public struct TransactionBody: Serializable, TextEnvelopable, Equatable {
    ...
    public init(
        ...
        if let withdrawals = withdrawals {
            dictionary[key(.withdrawals)] = try! withdrawals.toPrimitive()
        }
        ...
        if let scriptDataHash = scriptDataHash {
            dictionary[key(.scriptDataHash)] = scriptDataHash.toPrimitive()
        }
        if let collateral = collateral {
            dictionary[key(.collateral)] = try! collateral.toPrimitive()
        }
        ...
        if let votingProcedures = votingProcedures {
            dictionary[key(.votingProcedures)] = try! votingProcedures.toPrimitive()
        }
    ...
    public init(from primitive: Primitive) throws {
        ...
        switch outputsPrimitive {
        case .list(let outputsList):
            outputs = try outputsList.map { try TransactionOutput(from: $0) }
        case .indefiniteList(let outputsList):
            outputs = try outputsList.map { try TransactionOutput(from: $0) }
        default:
        ...
        let fee: Coin
        if case .uint(let feeValue) = feePrimitive {
            fee = Coin(feeValue)
        } else {
        ...
        if let certificatesPrimitive = primitiveDict[key(.certificates)] {
        ...
        if let withdrawalsPrimitive = primitiveDict[key(.withdrawals)] {
        ...
        let update: Update?
        if let updatePrimitive = primitiveDict[key(.update)] {
        ...
        if let auxiliaryDataHashPrimitive = primitiveDict[key(.auxiliaryDataHash)] {
        ...
        let mint: MultiAsset?
        if let mintPrimitive = primitiveDict[key(.mint)] {
        ...
        if let scriptDataHashPrimitive = primitiveDict[key(.scriptDataHash)] {
        ...
        if let collateralPrimitive = primitiveDict[key(.collateral)] {
        ...
        if let requiredSignersPrimitive = primitiveDict[key(.requiredSigners)] {
            requiredSigners = try ListOrNonEmptyOrderedSet<VerificationKeyHash>(
                from: requiredSignersPrimitive)
        } else {
            requiredSigners = nil
        }

        let networkId: Int?
        if let networkIdPrimitive = primitiveDict[key(.networkId)],
            case .uint(let networkIdValue) = networkIdPrimitive
        {
        ...
        if let collateralReturnPrimitive = primitiveDict[key(.collateralReturn)] {
        ...
        if let totalCollateralPrimitive = primitiveDict[key(.totalCollateral)],
            case .uint(let totalCollateralValue) = totalCollateralPrimitive
        {
        ...
        if let referenceInputsPrimitive = primitiveDict[key(.referenceInputs)] {
            referenceInputs = try ListOrNonEmptyOrderedSet<TransactionInput>(
                from: referenceInputsPrimitive)
        } else {
            referenceInputs = nil
        }

        let votingProcedures: VotingProcedures?
        if let votingProceduresPrimitive = primitiveDict[key(.votingProcedures)] {
        ...
        if let proposalProceduresPrimitive = primitiveDict[key(.proposalProcedures)] {
        ...
        if let currentTreasuryAmountPrimitive = primitiveDict[key(.currentTreasuryAmount)],
            case .uint(let currentTreasuryAmountValue) = currentTreasuryAmountPrimitive
        {
        ...
        if let treasuryDonationPrimitive = primitiveDict[key(.treasuryDonation)] {
    ...
    public static func fromDict(_ dict: Primitive) throws -> TransactionBody {
        ...
        guard let outputsPrimitive = orderedDict[.string(CodingKeys.outputs.stringValue)] else {
            throw CardanoCoreError.deserializeError("Missing or invalid outputs in TransactionBody")
        }
        let outputsList: [Primitive]
        switch outputsPrimitive {
        case .list(let l): outputsList = l
        case .indefiniteList(let l): outputsList = Array(l)
        default:
            throw CardanoCoreError.deserializeError("Missing or invalid outputs in TransactionBody")
        }
        ...
        guard let feePrimitive = orderedDict[.string(CodingKeys.fee.stringValue)],
            case .int(let feeValue) = feePrimitive
        else {
        ...
        if let auxiliaryDataHashPrimitive = orderedDict[
            .string(CodingKeys.auxiliaryDataHash.stringValue)]
        {
            auxiliaryDataHash = try AuxiliaryDataHash(from: auxiliaryDataHashPrimitive)
        }
        ...
        if let collateralPrimitive = orderedDict[.string(CodingKeys.collateral.stringValue)] {
            collateral = try ListOrNonEmptyOrderedSet<TransactionInput>.fromDict(
                collateralPrimitive)
        }

        var requiredSigners: ListOrNonEmptyOrderedSet<VerificationKeyHash>? = nil
        if let requiredSignersPrimitive = orderedDict[
            .string(CodingKeys.requiredSigners.stringValue)]
        {
            requiredSigners = try ListOrNonEmptyOrderedSet<VerificationKeyHash>.fromDict(
                requiredSignersPrimitive)
        }
        ...
        if let collateralReturnPrimitive = orderedDict[
            .string(CodingKeys.collateralReturn.stringValue)]
        {
            collateralReturn = try TransactionOutput.fromDict(collateralReturnPrimitive)
        }

        var totalCollateral: Coin? = nil
        if let totalCollateralPrimitive = orderedDict[
            .string(CodingKeys.totalCollateral.stringValue)],
            case .int(let totalCollateralValue) = totalCollateralPrimitive
        {
            totalCollateral = Coin(totalCollateralValue)
        }

        var referenceInputs: ListOrNonEmptyOrderedSet<TransactionInput>? = nil
        if let referenceInputsPrimitive = orderedDict[
            .string(CodingKeys.referenceInputs.stringValue)]
        {
            referenceInputs = try ListOrNonEmptyOrderedSet<TransactionInput>.fromDict(
                referenceInputsPrimitive)
        }

        var votingProcedures: VotingProcedures? = nil
        if let votingProceduresPrimitive = orderedDict[
            .string(CodingKeys.votingProcedures.stringValue)]
        {
            votingProcedures = try VotingProcedures.fromDict(votingProceduresPrimitive)
        }

        var proposalProcedures: ProposalProcedures? = nil
        if let proposalProceduresPrimitive = orderedDict[
            .string(CodingKeys.proposalProcedures.stringValue)]
        {
            proposalProcedures = try ProposalProcedures.fromDict(proposalProceduresPrimitive)
        }

        var currentTreasuryAmount: Coin? = nil
        if let currentTreasuryAmountPrimitive = orderedDict[
            .string(CodingKeys.currentTreasuryAmount.stringValue)],
            case .int(let currentTreasuryAmountValue) = currentTreasuryAmountPrimitive
        {
            currentTreasuryAmount = Coin(currentTreasuryAmountValue)
        }

        var treasuryDonation: PositiveCoin? = nil
        if let treasuryDonationPrimitive = orderedDict[
            .string(CodingKeys.treasuryDonation.stringValue)]
        {
    ...
    public func toDict() throws -> Primitive {
        ...
        func listOrNonEmptySetToList<T: CBORSerializable>(_ value: ListOrNonEmptyOrderedSet<T>)
            throws -> Primitive
        {
            switch value {
            case .list(let array):
        ...
        if let auxiliaryDataHash = auxiliaryDataHash {
            dict[.string(CodingKeys.auxiliaryDataHash.stringValue)] = .string(
        ...
        if let proposalProcedures = proposalProcedures {
            dict[.string(CodingKeys.proposalProcedures.stringValue)] =
                try proposalProcedures.toDict()
        }
        if let currentTreasuryAmount = currentTreasuryAmount {
    ...
    public static func == (lhs: TransactionBody, rhs: TransactionBody) -> Bool {
        return lhs.inputs == rhs.inputs && lhs.outputs == rhs.outputs && lhs.fee == rhs.fee
            && lhs.ttl == rhs.ttl && lhs.certificates == rhs.certificates
            && lhs.withdrawals == rhs.withdrawals && lhs.update == rhs.update
            && lhs.auxiliaryDataHash == rhs.auxiliaryDataHash
            && lhs.validityStart == rhs.validityStart && lhs.mint == rhs.mint
            && lhs.scriptDataHash == rhs.scriptDataHash && lhs.collateral == rhs.collateral
            && lhs.requiredSigners == rhs.requiredSigners && lhs.networkId == rhs.networkId
